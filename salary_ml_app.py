...
def main():
    ...
    activity = ["eda", "prediction", "countries","metrics"]
    ...
    if choice == 'eda':
        ...
        if st.checkbox("Preview Dataset"):
            number = st.number_input("Number to Show", min_value = 1, max_value = 20, value = 1, step = 1)
    ...
    # PREDICTION
        ...
        capital_gain = st.number_input("Captial Gain", 0.0, 99999.0)
        capital_loss = st.number_input("Capital Loss", 0.0, 4356.0)
        hours_per_week = st.number_input("Hours Per Week", 1.0, 99.0)
        ...
        selected_options = [age , workclass ,fnlwgt , education ,education_num , marital_status , occupation , relationship , race ,k_sex , capital_gain , capital_loss ,hours_per_week , native_country]
        ...
        if st.checkbox("Make Prediction"):
            ...
            if st.button("Predict"):
                ...
                monitor.add_data()
                st.success("Prediction Salary as:: {}".format(final_result))

    #COUNTRIES

    elif choice == 'countries':
        st.subheader("Countries Section")

        # List of Countries
        d_native_country = {"Canada": 0, "Philippines": 1, "Thailand": 2, "Scotland": 3, "Germany": 4, "Portugal": 5, "India": 6, "China": 7, "Japan": 8, "Peru": 9, "France": 10, "Greece": 11, "Taiwan": 12, "Laos": 13, "Hong": 14, "El-Salvador": 15, "Outlying-US(Guam-USVI-etc)": 16, "Yugoslavia": 17, "Cambodia": 18, "Italy": 19, "Honduras": 20, "Puerto-Rico": 21, "Dominican-Republic": 22, "Vietnam": 23, "Poland": 24, "Hungary": 25, "Holand-Netherlands": 26, "Ecuador": 27, "South": 28, "Guatemala": 29, "United-States": 30, "Nicaragua": 31, "Trinadad&Tobago": 32, "Cuba": 33, "Jamaica": 34, "Iran": 35, "?": 36, "Haiti": 37, "Columbia": 38, "Mexico": 39, "England": 40, "Ireland": 41}
        selected_countries = st.selectbox("Choose A Counry", tuple(d_native_country.keys()))
        # Selection Country
